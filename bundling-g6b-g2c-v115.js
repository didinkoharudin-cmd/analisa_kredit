/* V115 — Bundling G6B/G2C dengan UI Interaktif, promo per produk, download dan share WA. */
(function(){
'use strict';
...
let modal,selected=null,snapshot=null,stage='search',mode='NEW',searchQuery='',previousProduct='G6B',previousOverflow='';
...
function renderSearch(){
 stage='search';const rows=searchQuery.trim().length>=2?eligibleRows(searchQuery,mode):[];
 ...
 body.querySelector('[data-search]').oninput=e=>{searchQuery=e.target.value;renderSearch();requestAnimationFrame(()=>{const input=modal.querySelector('[data-search]');input?.focus();if(input)input.setSelectionRange(input.value.length,input.value.length);});};
 body.querySelectorAll('[data-deb]').forEach((button,i)=>button.onclick=()=>selectDebtor(rows[i]));
}
...
window.v115BundleRules={monthsUntil,bupInfo,source,productCode,access,eligibility,renewalCapacity,allocate,annuityInstallment,annuityGross,applyInsurancePromo};
